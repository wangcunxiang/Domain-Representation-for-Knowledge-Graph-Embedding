...
float *entityVec, *relationVec, *hRelVec, *tRelVec, *matrix_h, *matrix_t,*c_h,*c_t, *hEntDom, *tEntDom;
...
void init() {
    ...
    matrix_h = (float *)calloc(relationTotal * dimension * dimensionR, sizeof(float));
    matrix_t = (float *)calloc(relationTotal * dimension * dimensionR, sizeof(float));

    matrix_t = matrix_h + relationTotal *dimension *(dimension+1)/2+dimension;

    c_h = (float *)calloc(relationTotal * dimension *2, sizeof(float));
    c_t = c_h+relationTotal * dimension+dimension;
    ...
    for (long i = lef; i < rig; i++) {
        for (long j = 0; j < relationTotal; j++) {
            ...
            for (long k = 0; k < dimensionR; k++)
                ...
                {
                    hRelVec[last + k] += matrix_h[j * dimension * dimensionR + k * dimension + kk] * entityVec[i * dimension + kk];
                    tRelVec[last + k] += matrix_t[j * dimension * dimensionR + k * dimension + kk] * entityVec[i * dimension + kk];
                }
			if(domain){
				hEntDom[ i * relationTotal + j] = cal_hEntDom(i,j);
				hEntDom[ i * relationTotal + j] = cal_tEntDom(i,j);
			}
        }
    }
    pthread_exit(NULL);
...
void prepare() {
    ...
    for (long i = 0; i < relationTotal; i++)
            for (long jj = 0; jj < dimension; jj++)
                for (long ii = 0; ii < dimensionR; ii++)
                    tmp = fscanf(fin, "%f", &matrix_h[i * dimensionR * dimension + jj + ii * dimension]);
    fclose(fin);
    fin = fopen((initPath + "B" + note + ".vec").c_str(), "r");
    for (long i = 0; i < relationTotal; i++)
            for (long jj = 0; jj < dimension; jj++)
                for (long ii = 0; ii < dimensionR; ii++)
                    tmp = fscanf(fin, "%f", &matrix_t[i * dimensionR * dimension + jj + ii * dimension]);
    fclose(fin);

    cout<<"init done\n";

    fin = fopen((loadPath + "SM" + note + ".vec").c_str(), "r");
    ...
    fclose(fin);
     ...
     fin = fopen((loadPath + "SC" + note + ".vec").c_str(), "r");
    for (long i = 0; i < relationTotal*2; i++) {
        long last = i * dimension;
        for (long j = 0; j < dimension; j++)
            tmp = fscanf(fin, "%f", &c_h[last + j]);
    }
    fclose(fin);

    cout<<"load done\n";

    hRelVec = (float *)calloc(entityTotal * relationTotal * dimensionR,sizeof(float));
    tRelVec = (float *)calloc(entityTotal * relationTotal * dimensionR,sizeof(float));
...
float calc_value(long e1, long e2, long rel) {
    float res = 0;
    ...
    for (long i = 0; i < dimensionR; i++)
        res += fabs(hRelVec[last1 + i] + relationVec[lastr + i] - tRelVec[last2 + i]);
    return res;
}
...
REAL cal_k_h(INT e,INT rel, REAL *M){
    ...
    for(INT i=0;i<dimension;i++)
        for(INT j=i;j<dimension;j++){
            k+=2*M[i * dimension + j -  i*(i+1)/2]*(hRelVec[lasta+i]-c_h[lastc+i])*(hRelVec[lasta+j]-c_h[lastc+j]);
        }
...
REAL cal_k_t(INT e,INT rel, REAL *M){
    ...
    for(INT i=0;i<dimension;i++)
        for(INT j=i;j<dimension;j++)
            k+=2*M[i * dimension + j -  i*(i+1)/2]*(tRelVec[lasta+i]-c_t[lastc+i])*(tRelVec[lasta+j]-c_t[lastc+j]);
    return k;
}
...
REAL cal_y_h(INT e,INT rel){
    ...
    REAL y=0;
    for(INT i=0;i<dimension;i++)
        y+=(hRelVec[lasta+i]-c_h[lastc+i])*(hRelVec[lasta+i]-c_h[lastc+i]);
    return y;
}
REAL cal_y_t(INT e,INT rel){
    ...
    REAL y=0;
    for(INT i=0;i<dimension;i++)
        y+=(tRelVec[lasta+i]-c_t[lastc+i])*(tRelVec[lasta+i]-c_t[lastc+i]);
    return y;
}
REAL cal_hEntDom(long eh,long rel)
{
...
//int i=0;
float calc_sum(long eh,long et,long rel){
   // cout<<"cv\n"<<endl;
    float res=calc_value(eh,et,rel);
	float d_h = 0,d_t = 0;
    if(domain){
