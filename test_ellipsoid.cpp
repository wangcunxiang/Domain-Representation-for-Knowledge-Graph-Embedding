...
long dimension = 100;
float testid = 0;
float *entityVec, *relationVec, *matrix_h,*matrix_t,*centr_h,*centr_t,*hEntDom,*tEntDom;
...
void* prepareMode(void *con) {
    ...
    for (long i = lef; i < rig; i++) {
        for (long j = 0; j < relationTotal; j++) {
			if(domain){
				hEntDom[ i * relationTotal + j] = cal_hEntDom(i,j);printf("%d %d %d:161\n",id,i,j);
				float tmp1=cal_tEntDom(i,j);printf("%d %d %d:162\n",id,i,j);//printf("%.2f\n",tEntDom[ i * relationTotal + j]);
				tEntDom[ i * relationTotal + j] = tmp1;printf("%d %d %d:163\n",id,i,j);
			}
        }
    }
    pthread_exit(NULL);
...
REAL cal_tEntDom(long et,long rel)
{
    ...
    REAL k_t=cal_k_t(et, rel, M_t);printf("%.2f\n",k_t);
    REAL y_t=cal_y_t(et, rel);printf("%.2f\n",y_t);
	REAL d_t;
	REAL tmp2=1-pow(k_t,-0.5);printf("%.2f\n",tmp2);
    if(tmp2<0)
        d_t=0;
    else
        d_t=tmp2*tmp2*y_t;
